...
<div class="container">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h2 class="mb-0">{{ __('messages.create_material') }} for "{{ $chapter->title }}"</h2>
            <a href="{{ route('courses.show', $course) }}" class="btn btn-outline-secondary">{{ __('messages.go_back') }}</a>
        </div>
        <div class="card-body">
            <form action="{{ route('chapters.materials.store', [$course, $chapter]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
